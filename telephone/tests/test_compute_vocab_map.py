...
from telephone.tests.test_constants import US_MAP, LOWERCASE_ALPHA
...
def test_compute_vocab_map(vocabulary: Set[str]) -> None:
    ...
    vocab_map = compute_vocab_map(vocabulary, US_MAP)
    for wordhash, words in vocab_map.items():
        for word in words:
            implicit_format = "0" * len(word)
            assert words_to_number(word, US_MAP, implicit_format) == wordhash
